...
| Husky + pre-commit + gitleaks                     | **Implement now**                         | Deterministic git hooks: lint-staged (formatting) + npm test (Jest unit tests only — Playwright E2E stays in CI, not pre-commit, to keep commits fast) + lightweight secret scan. Always runs, regardless of which tool edited the code.                                  |
