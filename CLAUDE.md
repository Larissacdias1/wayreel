...
- If, during a task, you identify that the correct fix requires going beyond what was explicitly asked (e.g., changing a value the instruction said to preserve), STOP and ask before acting — even if the extension seems obviously right. Explain what you found and why you think it needs to change, and wait for confirmation, unless the task has already explicitly authorized that kind of judgment call. This has already happened twice in this session (renaming states during translation, adding `implements FlightProvider` in mock-flight-adapter.ts) — in both cases the change was technically correct, but should have been asked BEFORE being made, not reported afterward. Being right on the merits doesn't replace the pause — even an additive, low-risk change to a file outside the current issue's scope requires prior confirmation.
- When integrating external libraries with asynchronous state (MapLibre, WebGL, etc.), never rely solely on events (`load`, `moveend`) as proof that an action took effect — measure the actual resulting state (`getCenter`/`getZoom`/`getPitch`, or equivalent) before considering an integration verified. This has already caused 3 silent bugs in this session (`setWorkerUrl`, premature `moveend`, implicit `maxPitch`).
