...
> RESOLVED (#126): `validateIntent` is implemented in `src/agent/nodes.ts` and wired into `src/agent/graph.ts` — it checks the required fields (origin_iata, budget_level, vibe, departure_date) and sets `clarification_needed`/`missing_info` accordingly, feeding `shouldClarify` (Section 8.3) directly, rather than living as a separate board issue.

> RESOLVED (#126): `validateRecommendation` is implemented in `src/agent/nodes.ts` and wired into `src/agent/graph.ts` — it confirms the recommended `destination_id` actually matches one of the retrieved destinations (catching a hallucinated ID), while the confidence >= 0.6 threshold routing remains `hasRecommendation`'s job (Section 8.3), rather than living as a separate board issue.
...
- Problem: verification of code depending on live external APIs (Gemini embeddings, in issues #101, #105, #106, #107) was handled ad hoc via temporary scripts each time, without ever being registered as an explicit pattern — risking a future session writing a permanent Jest test that calls a live API by mistake.
- Alternatives evaluated: writing permanent Jest tests against the real API (rejected — makes npm test, which runs in pre-commit, dependent on network access and API quota for every single commit, even unrelated ones; also non-deterministic if the API's output format or values shift); mocking the API response permanently in Jest (viable for pure logic, already the approach for domain/schemas and domain/airport-codes, but doesn't verify the real integration ever actually works).
- Decision: pure/deterministic logic gets permanent Jest tests with fixed inputs (schemas, parsers, cosine similarity). Anything that must call a live external API is verified once, manually, via a temporary script that is deleted after confirming output — never becomes part of the automated suite. This applies going forward to Sprint 2's MCP/flight-provider integration as well.
